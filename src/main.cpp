...
#if defined(NDEBUG)
#error "CPTS cannot be compiled without assertions."
#endif
...
{
    //Check to see if the zCPTS is properly signed
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zCPTS spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zCPTS without the correct spend type. txid=%s", __func__,
                tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zCPTS spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zCPTS spend with serial %s from tx %s is not in valid range\n", __func__,
    ...
    for (const CTxIn& txin : tx.vin) {
...

            {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                        return state.Invalid(error("AcceptToMemoryPool : zCPTS spend tx %s already in block %d",
                    ...
                    // Check that zCPTS mints are not already known
                    if (tx.IsZerocoinMint()) {
                        for (auto& out : tx.vout) {
                            if (!out.IsZerocoinMint())
                                continue;
        ...
        int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
        {
            ...
            if (nHeight == 0) {
                ...
                ret = blockValue / 10 * 7.5; //70%
            } else if (nHeight <= 50000 && nHeight > 20000) {
                ret = blockValue / 10 * 5.5; //50%
            } else if (nHeight <= 100000 && nHeight > 50000) {
                ret = blockValue / 10 * 5; //40%
            } else if (nHeight <= 150000 && nHeight > 100000) {
                ret = blockValue / 10 * 4; //30%
            } else if (nHeight > 150000) {
                ret = blockValue / 10 * 70; //60%
            }
		...
		*/
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from CPTS
        ...
        {
            RenameThread("cpts-scriptch");
            scriptcheckqueue.Thread();
        }

        void RecalculateZCPTSMinted()
        {
        ...
        void RecalculateZCPTSSpent()
        {
            ...
            while (true) {
                ...
                //Remove spends from zCPTS supply
        ...
        bool RecalculateCPTSSupply(int nHeightStart)
        {
        ...
        bool UpdateZCPTSSupply(const CBlock& block, CBlockIndex* pindex)
        {
            std::list<CZerocoinMint> listMints;
            BlockToZerocoinMintList(block, listMints);
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            UpdateZCPTSSupply(block, pindex);
            ...
            //    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zCPTSSpent: %s\n",
 ...
 */
        ...
        void static UpdateTip(CBlockIndex * pindexNew)
        {
            ...
            // If turned on AutoZeromint will automatically convert CPTS to zCPTS
 ...
 */
        ...
        bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
        {
            ...
            if (nHeight > Params().Zerocoin_StartHeight()) {
                ...
                for (const CTransaction& tx : block.vtx) {
                    ...
                    // double check that there are no double spent zCPTS spends in this block
                    if (tx.IsZerocoinSpend()) {
                        for (const CTxIn txIn : tx.vin) {
                            if (txIn.scriptSig.IsZerocoinSpend()) {
                                libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                                if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                                    return state.DoS(100, error("%s : Double spending of zCPTS serial %s in block\n Block: %s",
        ...
        bool ProcessNewBlock(CValidationState & state, CNode * pfrom, CBlock * pblock, CDiskBlockPos * dbp)
        {
            ...
            if (nMints || nSpends)
                LogPrintf("%s : block contains %d zCPTS mints and %d zCPTS spends\n", __func__, nMints, nSpends);
        ...
        bool static ProcessMessage(CNode * pfrom, string strCommand, CDataStream & vRecv, int64_t nTimeReceived)
        {
            ...
            if (strCommand == NetMsgType::VERSION) {
                ...
                if (pfrom->cleanSubVer == "/CPTS Core:0.0.0.1/") { // Archived pivx 2.3 source
                    LOCK(cs_main);
                ...
                // CPTS: We use certain sporks during IBD, so check to see if they are
