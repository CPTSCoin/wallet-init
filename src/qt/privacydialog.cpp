                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zCPTSpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    

    // "Spending 999999 zCPTS ought to be enough for anybody." - Bill Gates, 2017
    ui->zCPTSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCPTSSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCPTSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCPTS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_23_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCPTS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCPTS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCPTS in ") +
...
void PrivacyDialog::on_pushButtonSpendzCPTS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_23_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCPTS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCPTS
        sendzCPTS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCPTS
    sendzCPTS();
}
...
{
    ui->labelzCPTSSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCPTS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid CPTS Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCPTSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCPTS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CPTS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCPTS</b>";
    ...
    // use mints from zCPTS selector if applicable
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zCPTS require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zCPTS"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZCPTS_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zCPTS require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zCPTS"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zCPTS was spent successfully update the addressbook with the label
    ...
    // Clear zabet selector in case it was used
    ZAbetControlDialog::setSelectedMints.clear();
    ui->labelzCPTSSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCPTS Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Abet, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCPTS Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCPTS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCPTS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCPTS "));
    ui->labelzCPTSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCPTSSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK23Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzCPTS->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zCPTS
        ui->pushButtonMintzCPTS->setEnabled(false);
        ui->pushButtonMintzCPTS->setToolTip(tr("zCPTS is currently disabled due to maintenance."));

        // Spend zCPTS
        ui->pushButtonSpendzCPTS->setEnabled(false);
        ui->pushButtonSpendzCPTS->setToolTip(tr("zCPTS is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zCPTS
        ui->pushButtonMintzCPTS->setEnabled(true);
        ui->pushButtonMintzCPTS->setToolTip(tr("PrivacyDialog", "Enter an amount of CPTS to convert to zCPTS", 0));

        // Spend zCPTS
        ui->pushButtonSpendzCPTS->setEnabled(true);
        ui->pushButtonSpendzCPTS->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}