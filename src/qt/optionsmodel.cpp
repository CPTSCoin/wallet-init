...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CPTS);
    ...
    if (!settings.contains("nAnonymizeCPTSAmount"))
        settings.setValue("nAnonymizeCPTSAmount", 1000);

    nAnonymizeCPTSAmount = settings.value("nAnonymizeCPTSAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCPTSAmount"))
        SoftSetArg("-anonymizecptsamount", settings.value("nAnonymizeCPTSAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cpts.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCPTSAmount:
            return QVariant(nAnonymizeCPTSAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCPTSAmount:
            nAnonymizeCPTSAmount = value.toInt();
            settings.setValue("nAnonymizeCPTSAmount", nAnonymizeCPTSAmount);
            emit anonymizeCPTSAmountChanged(nAnonymizeCPTSAmount);
            break;
        case CoinControlFeatures:
