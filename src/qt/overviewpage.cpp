...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::CPTS)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    QString automintHelp = tr("Current percentage of zCPTS.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in cpts.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in cpts.conf");
    }
    ...
    bool showCPTSAvailable = settingShowAllBalances || abetAvailableBalance != nTotalBalance;
    bool showWatchOnlyCPTSAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showCPTSPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyCPTSPending = watchUnconfBalance != 0;
    bool showCPTSLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyCPTSLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;

    ui->labelBalance->setVisible(showCPTSAvailable || showWatchOnlyCPTSAvailable);
    ui->labelBalanceText->setVisible(showCPTSAvailable || showWatchOnlyCPTSAvailable);
    ui->labelWatchAvailable->setVisible(showCPTSAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showCPTSPending || showWatchOnlyCPTSPending);
    ui->labelPendingText->setVisible(showCPTSPending || showWatchOnlyCPTSPending);
    ui->labelWatchPending->setVisible(showCPTSPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showCPTSLocked || showWatchOnlyCPTSLocked);
    ui->labelLockedBalanceText->setVisible(showCPTSLocked || showWatchOnlyCPTSLocked);
    ui->labelWatchLocked->setVisible(showCPTSLocked && showWatchOnly);
    ui->labelImmature->setVisible(showImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelImmatureText->setVisible(showImmature || showWatchOnlyImmature);
    ui->labelWatchImmature->setVisible(showImmature && showWatchOnly);
	bool showzCPTSAvailable = settingShowAllBalances;
	bool showzCPTSUnconfirmed = settingShowAllBalances;
	bool showzCPTSImmature = settingShowAllBalances;
...
void OverviewPage::timerTickSlot()
{
    ...
    QNetworkReply* replyEx = managerEx.get(QNetworkRequest(QUrl(QString("https://api.crypto-points.io/exchange2/%1.png").arg(unixtime))));
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("CPTS")
    updateDisplayUnit();
}
...
// TFinch has edited it to work with CPTS
...
void OverviewPage::updatBlockChainInfo()
{
    if (masternodeSync.IsBlockchainSynced()) {
        ...
        ui->label_Supply_value->setText(QString::number(chainActive.Tip()->nMoneySupply / COIN).append(" CPTS"));
