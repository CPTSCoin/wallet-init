...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total cpts balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zCPTSsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zCPTS denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zCPTS denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zCPTS denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zCPTS denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zCPTS denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zCPTS denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zCPTS denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zCPTS denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zCPTS denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in cpts/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in cpts/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zabetObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zabetObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zCPTSsupply", zabetObj));
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are cpts addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) cpts address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"cptsaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"cptsaddress\"  (string, required) The cpts address to use for the signature.\n"
