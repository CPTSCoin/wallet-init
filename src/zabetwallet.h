...
#ifndef ALTBET_ZCPTSWALLET_H
#define ALTBET_ZCPTSWALLET_H
...
public:
    CzCPTSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCPTS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCPTS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ALTBET_ZCPTSWALLET_H
