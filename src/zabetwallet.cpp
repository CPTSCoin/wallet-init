...
CzCPTSWallet::CzCPTSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zabet seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCPTSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCPTSSeed_deprecated()) {
                    LogPrintf("%s: Updated zCPTS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zabet seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zabet wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCPTSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCPTSCount(nCountLastUsed);
    else if (!walletdb.ReadZCPTSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCPTSWallet::Lock()
{
    seedMaster = 0;
}

void CzCPTSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCPTSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCPTS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCPTSWallet::LoadMintPoolFromDB()
{
...
void CzCPTSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCPTSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCPTSWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzCPTSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCPTS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzCPTSWallet::SeedToZCPTS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCPTSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCPTSWallet::GenerateDeterministicZCPTS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCPTSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCPTSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
