...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or cpts.conf setting gen)\n"
...
UniValue prioritisetransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "                  (priority of a transaction is calculated: coinage * value_in_uabet / txsize) \n"
            "3. fee delta      (numeric, required) The fee value (in uabet) to add (or subtract, if negative).\n"
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "      \"flags\" : \"flags\"            (string) \n"
            "  },\n"
            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in uabet)\n"
    ...
    if (vNodes.empty() && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "CPTS is not connected!");

    if (IsInitialBlockDownload() && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "CPTS is downloading blocks...");
