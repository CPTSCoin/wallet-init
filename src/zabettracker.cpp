...
CzCPTSTracker::CzCPTSTracker(std::string strWalletFile)
{
...
CzCPTSTracker::~CzCPTSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCPTSTracker::Init()
{
...
bool CzCPTSTracker::Archive(CMintMeta& meta)
{
...
bool CzCPTSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCPTSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCPTSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCPTSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCPTSTracker::GetSerialHashes()
{
...
CAmount CzCPTSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCPTSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCPTSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCPTSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCPTSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCPTSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCPTSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCPTSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCPTSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCPTSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCPTSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCPTSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCPTSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCPTSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCPTSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCPTSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCPTSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzabet from DB\n", __func__, listDeterministicDB.size());
    }
