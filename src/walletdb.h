...
public:
    ...
    bool WriteZCPTSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCPTSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCPTSSeed_deprecated(uint256& seed);
    bool EraseZCPTSSeed();
    bool EraseZCPTSSeed_deprecated();

    bool WriteZCPTSCount(const uint32_t& nCount);
    bool ReadZCPTSCount(uint32_t& nCount);
