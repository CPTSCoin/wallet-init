...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 CPTS at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCPTS send
enum ZerocoinSpendStatus {
    ZCPTS_SPEND_OKAY = 0,                            // No error
    ZCPTS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCPTS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCPTS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCPTS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCPTS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCPTS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCPTS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCPTS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCPTS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCPTS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCPTS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCPTS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCPTS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCPTS_SPENT_USED_ZCPTS = 14,                      // Coin has already been spend
    ZCPTS_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZCPTS_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    bool CreateZCPTSOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzCPTSWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzCPTSTracker> zabetTracker;
    ...
    void setZWallet(CzCPTSWallet* zwallet)
    {
        zwalletMain = zwallet;
        zabetTracker = std::unique_ptr<CzCPTSTracker>(new CzCPTSTracker(strWalletFile));
    }

    CzCPTSWallet* getZWallet() { return zwalletMain; }
