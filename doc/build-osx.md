...
====================================
This guide will show you how to build cptsd (headless client) for OSX.
...
5.  (Optional) You can also install cptsd to your path:
...
4. Enter "cpts-qt" as project name, enter src/qt as location
...
------------------------
You can ignore this section if you are building `cptsd` for your own use.

cptsd/cpts-cli binaries are not included in the cpts-Qt.app bundle.

If you are building `cptsd` or `cpts-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the CPTS-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./cptsd`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./cptsd` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=cptsrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/CPTS/cpts.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/CPTS/cpts.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/CPTS/debug.log

Other commands:
-------

    ./cptsd -daemon # to start the cpts daemon.
    ./cpts-cli --help  # for a list of command-line options.
    ./cpts-cli help    # When the daemon is running, to get a list of RPC commands
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure CPTS with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the CPTS wallet:
...
You have the choice to build the GUI CPTS wallet as a Mac OSX app, described in “How to build the CPTS-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the CPTS-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find CPTS-Qt.dmg inside your CPTS folder. Open and install the wallet like any typical Mac app.
