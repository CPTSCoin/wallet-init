...
VERSION=
commit=false
url=https://github.com/cptsproject/cpts
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the cpts, gitian-builder, gitian.sigs, and cpts-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/cptsproject/cpts
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit cpts=${COMMIT} --url cpts=${url} ../cpts/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../cpts/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/cpts-*.tar.gz build/out/src/cpts-*.tar.gz ../cpts-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit cpts=${COMMIT} --url cpts=${url} ../cpts/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/cpts-*.zip build/out/cpts-*.exe ../cpts-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit cpts=${COMMIT} --url cpts=${url} ../cpts/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/cpts-*.tar.gz build/out/cpts-*.dmg ../cpts-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit cpts=${COMMIT} --url cpts=${url} ../cpts/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../cpts/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/cpts-*.tar.gz build/out/src/cpts-*.tar.gz ../cpts-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../cpts/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/cpts-*win64-setup.exe ../cpts-binaries/${VERSION}
	    mv build/out/cpts-*win32-setup.exe ../cpts-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../cpts/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/cpts-osx-signed.dmg ../cpts-binaries/${VERSION}/cpts-${VERSION}-osx.dmg
	fi
	popd || exit
