...
class TestNode():
    ...
    def __init__(self, i, datadir, rpchost, timewait, binary, stderr, mocktime, coverage_dir, extra_conf=None, extra_args=None, use_cli=False):
        ...
        if timewait:
            ...
            self.rpc_timeout = 60
        if binary is None:
            self.binary = os.getenv("ALTBETD", "cptsd")
        else:
            self.binary = binary
        self.stderr = stderr
        ...
        self.cli = TestNodeCLI(os.getenv("ALTBETCLI", "cpts-cli"), self.datadir)
    ...
    def start(self, extra_args=None, stderr=None, *args, **kwargs):
        ...
        self.running = True
        self.log.debug("cptsd started, waiting for RPC to come up")
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError('cptsd exited with status {} during initialization'.format(self.process.returncode))
            try:
        ...
        raise AssertionError("Unable to connect to cptsd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                self.log.debug('cptsd failed to start: %s', e)
                self.running = False
                self.process = None
                ...
                if expected_msg is None:
                    assert_msg = "cptsd should have exited with an error"
                else:
                    assert_msg = "cptsd should have exited with expected error " + expected_msg
                raise AssertionError(assert_msg)

    def node_encrypt_wallet(self, passphrase):
        """"Encrypts the wallet.

        This causes cptsd to shutdown, so this method takes
